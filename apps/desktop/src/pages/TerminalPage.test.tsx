...
describe("workspace-backed terminal page", () => {
  ...
  it("saves a customized SSH config projection while preserving its remembered session references", async () => {
    ...
    expect(catalog.hosts[2]).toMatchObject({
      ...
      connection_methods: [{ method_id: "ssh_config", ssh_config_alias: "only-in-ssh-config", target: { kind: "ssh", destination: "only-in-ssh-config" } }],
    });
    const document = api.updateWorkspace.mock.calls.slice(-1)[0][1] as WorkspaceDocument;
  ...
  it.each([
    ...
    expect(catalog.hosts.find((host) => host.host_id === host_id)).toMatchObject({
      ...
      connection_methods: [{ method_id: "ssh_config", ssh_config_alias: "only-in-ssh-config", target: {
    ...
    const target = {
      ...
      ssh_config_alias: "only-in-ssh-config",
