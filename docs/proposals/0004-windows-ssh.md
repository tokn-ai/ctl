...
`ctld.exe connect` command. The default Unix platform continues to select
`exec ctld connect`, regardless of the client's operating system. Windows hosts
currently require OpenSSH's default cmd.exe shell.

`ctld` relays binary stdio to the authenticated user's fixed rmux named pipe.
...
3. rmuxd owns every process and PTY in its sessions; ctld owns only the relay.
